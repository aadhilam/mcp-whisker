...
program
    ...
    try {
        ...
        if (options.filter || options.startTime || options.endTime) {
            ...
            const filteredLogs = await logFilterService.filterLogs(logs, options.filter, options.startTime, options.endTime);
...
program
    ...
    try {
        ...
        const logs = await calicoService.getFlowLogs();
        const filter = logFilterService.buildDeniedStagedPoliciesFilter(options.namespace, options.sourceName, options.destName);
...
program
    ...
    try {
        ...
        const logs = await calicoService.getFlowLogs();
        const filter = logFilterService.buildHighTrafficFilter(parseInt(options.minPackets), parseInt(options.minBytes));
        const filteredLogs = await logFilterService.filterLogs(logs, filter);
...
program
    ...
    const kubectlCheck = new Promise((resolve) => {
